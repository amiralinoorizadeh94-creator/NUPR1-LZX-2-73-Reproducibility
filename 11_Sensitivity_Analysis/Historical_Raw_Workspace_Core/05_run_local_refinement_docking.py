...
OUT=Path("refinement_docking_out"); OUT.mkdir(exist_ok=True)
rows=[]
recs=sorted(Path("refined_receptors").glob("*_R*.pdbqt"))
if not recs: raise RuntimeError("No refined receptor models found")

for i,rec in enumerate(recs,1):
    pts=coords(rec)
    xs=[p[0] for p in pts]; ys=[p[1] for p in pts]; zs=[p[2] for p in pts]
    center=(sum(xs)/len(xs),sum(ys)/len(ys),sum(zs)/len(zs))
    cfg=OUT/(rec.stem+".txt")
...
""",encoding="utf-8")
    ...
    out=OUT/(rec.stem+"_out.pdbqt")
    cp=subprocess.run([str(vina.resolve()),"--config",str(cfg),"--out",str(out)],
                      capture_output=True,text=True)
    txt=(cp.stdout or "")+"\n"+(cp.stderr or "")
    (OUT/(rec.stem+".log")).write_text(txt,encoding="utf-8")
    if cp.returncode!=0:
        print(txt); raise SystemExit(cp.returncode)
    ...
    print(f"[{i}/{len(recs)}] {rec.stem} best={best}")

with open(OUT/"summary.csv","w",newline="",encoding="utf-8") as f:
    w=csv.writer(f); w.writerow(["model","best_vina_kcal_mol"]); w.writerows(rows)
...
if vals:
    ...
    for cutoff in [-5.5,-6.0,-6.5,-7.0,-7.5,-8.0]:
        n=sum(v<=cutoff for v in vals)
        print(f"fraction <= {cutoff:.1f} = {n}/{len(vals)} ({100*n/len(vals):.1f}%)")
